...
Create new phase collapsed across phases 2-3
```{r}
sr_phase <- d %>%
...
dc <- d %>%
  ...
  group_by(SID, trial)%>%
  select(SID, trial, familiarity, num_objs, age_months, age_years, label_request, request_slide, slide_touch, request_touch, touch_response)%>%
...
Get phase lengths and plot to include. 
...
Separate mixed-effects logistic regression models for 3 phases.

Label-Request phase
```{r label, results="asis"}
lm <- glmer(social_ref ~ num_objs * familiarity * age_months +
              (1|SID),
              data = filter(dc,phase_name == "label_request", exclude == 0), 
              family = "binomial")
...
lm <- glmer(social_ref ~ num_objs * familiarity * age_months +
              (1|SID),
              data = filter(dc,phase_name == "request_touch", exclude == 0), 
              family = "binomial")
...
Touch-Response phase
```{r label, results="asis"}
lm <- glmer(social_ref ~ num_objs * familiarity * age_months +
              (1|SID),
              data = filter(dc,phase_name == "touch_response", exclude == 0), 
              family = "binomial")
...
Linear mixed effects regression predicting number of looks.
