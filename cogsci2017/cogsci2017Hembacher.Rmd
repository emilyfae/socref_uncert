...
accuracy <- read.csv("accuracy_e1.csv")
accuracy_key <- read.csv("accuracy_key.csv")
...
both_n_kids <- length(unique(both$ID))
both_n_trials <- length(both$ID)

acc_ms <- acc%>%
  filter(answer != "ambiguous")%>%
  group_by(ID, answer)%>%
