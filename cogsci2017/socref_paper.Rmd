...
The mean durations of the phases are presented in Figure \ref{fig:phases_e1}. The phases varied in length as expected with the label and response phases being longest. However, note that we are interested in comparing the amount of social referencing across ambiguity conditions and not across phases directly.

```{r phases_e1, fig.env = "figure", fig.pos = "h", fig.align='center', set.cap.width=T, fig.width=4, fig.height=2, fig.cap='Mean durations of trial phases in Experiment 1.'}
...
phase_lengths$phase_name <- factor(phase_lengths$phase_name, levels = c("label","slide", "planning", "response"))
...
```{r phases_e2, fig.env = "figure", fig.pos = "h", fig.align='center', set.cap.width=T, fig.width=4, fig.height=2, fig.cap='Mean durations of trial phases in Experiment 2.'}
...
The mean durations of the phases for Experiment 2 are presented in Figure \ref{fig:phases_e2}. They varied in length similar to Experiment 1. 
...
g_lm <- summary(glmer(acc ~ trial_type * gaze  +
                           (1| SID), 
                         data = lmer_data,
                       family = "binomial"))
```
...
acc_mod <- summary(lmer(num_looks ~ acc * age_c * phase_name +
                           (1 | SID), 
                         data = lmer_data))
```
...
msslooks_acc <- filter(d, exclude == 0, !is.na(acc)) %>%
  group_by(SID, phase_name, acc, age_years, Gaze) %>%
  summarise(num_looks = mean(num_looks))

mslooks_acc <- filter(d, exclude == 0, !is.na(acc)) %>%
  group_by(acc, phase_name, age_years, Gaze) %>%
  multi_boot_standard(col = "num_looks") 
