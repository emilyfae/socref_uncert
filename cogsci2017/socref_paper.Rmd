...
Experiment & Phase & Mean & SD \\ 
  \hline
  1 & Label & `r round(plength1$label$mean, digits = 2)` & `r round(plength1$label$sd, digits = 2)` \\ 
  1 & Slide & `r round(plength1$slide$mean, digits = 2)` & `r round(plength1$slide$sd, digits = 2)` \\ 
  1 & Planning & `r round(plength1$planning$mean, digits = 2)` & `r round(plength1$planning$sd, digits = 2)` \\ 
  1 & Response & `r round(plength1$response$mean, digits = 2)` & `r round(plength1$response$sd, digits = 2)` \\
  \hline
  2 & Label & `r round(plength2$label$mean, digits = 2)` & `r round(plength2$label$sd, digits = 2)` \\ 
  2 & Slide & `r round(plength2$slide$mean, digits = 2)` & `r round(plength2$slide$sd, digits = 2)` \\ 
  2 & Planning & `r round(plength2$planning$mean, digits = 2)` & `r round(plength2$planning$sd, digits = 2)` \\ 
  2 & Response & `r round(plength2$response$mean, digits = 2)` & `r round(plength2$response$sd, digits = 2)` \\ 
   \hline
\end{tabular}
...
acc_tab <- xtable::xtable(acc_table, digits = 2, 
                      caption = "Accuracy Rates.")

print(acc_tab, type="latex", comment = F, table.placement = "h",
      floating.environment = "table", include.rownames=TRUE)
```
...
tot <- 100*(length(d$acc[d$response_orig == "NC"]) + length(d$acc[d$response_orig == "LR"]))/n
```

We next examined children's accuracy (i.e., their rates of putting the correct item in the bucket for two-familiar trials, in which there was a correct response). Children sometimes put two items in the bucket (`r round(tot, digits = 0)`% of trials). If they put the correct item in the bucket first followed by the incorrect object, we counted their response as correct. If they put the incorrect object in first, we counted their response as incorrect. Children's accuracy is displayed in Table \ref{tab:xtable} . Overall children generally chose the correct item, indicating that they understood the task and were motivated to answer correctly.
...
To quantify the effects of trial type gaze condition, and age in months on the probability of accuracy, we fit the following mixed-effects logistic regression model: `correct ~ trial type * gaze + age + (1| subject)`. Accuracy was significantly lower for novel ($\beta$ = `r round(acc_mod$coefficients[2], digits = 2)`, *p* < .001) and mutual trials ($\beta$ = `r round(acc_mod$coefficients[3], digits = 2)`, *p* < .001) compared to familiar trials, and trial type interacted with gaze condition such that accuracy was significantly higher in the gaze condition for novel ($\beta$ = `r round(acc_mod$coefficients[6], digits = 2)`, *p* < .001) and mutual trials ($\beta$ = `r round(acc_mod$coefficients[7], digits = 2)`, *p* < .001). Age did not significantly predict accuracy ($\beta$ = `r round(acc_mod$coefficients[5], digits = 2)`, *p* = .12). 
...
To quantify the relation between accuracy and social referencing across phases, we fit a mixed-effects linear regression model with the following structure: `number of looks ~ accuracy * age in months * phase + (1 | subject)`. Results showed that accuracy and phase interacted such that children looked at the experimenter significantly more during the labeling phase of accurate trials ($\beta$ = `r round(acclooks_mod$coefficients[8], digits = 2)`, *p* < .001), and significantly less in the response phase of accurate trials ($\beta$ = `r round(acclooks_mod$coefficients[10], digits = 2)`, *p* < .001). There was also a three-way interaction between accuracy, age, and phase such that there was significantly less looking to the experimenter in the response phase of accurate trials as age increased ($\beta$ = `r round(acclooks_mod$coefficients[16], digits = 2)`, *p* < .01).
