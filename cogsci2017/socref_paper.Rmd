...
Experiment & Phase & Mean & SD \\ 
  \hline
  1 & Label & `r plength1$label$mean` & `r plength1$label$sd` \\ 
  1 & Slide & `r plength1$slide$mean` & `r plength1$slide$sd` \\ 
  1 & Planning & `r plength1$planning$mean` & `r plength1$planning$sd` \\ 
  1 & Response & `r plength1$response$mean` & `r plength1$response$sd` \\
  2 & Label & `r plength2$label$mean` & `r plength2$label$sd` \\ 
  2 & Slide & `r plength2$slide$mean` & `r plength2$slide$sd` \\ 
  2 & Planning & `r plength2$planning$mean` & `r plength2$planning$sd` \\ 
  2 & Response & `r plength2$response$mean` & `r plength2$response$sd` \\ 
   \hline
\end{tabular}
\caption{Phase durations for Experiments 1 and 2.\label{tab:phases}} 
\end{table}

However, note that we are interested in comparing the amount of social referencing across ambiguity conditions and not across phases directly.
...
```{r acc_tab, results="asis", eval = FALSE}
##table is not printing, not sure why
acc_tab <- xtable::xtable(acc_table, digits = 2, 
                      caption = "Accuracy Rates.")

print(acc_tab, type="latex", comment = F, table.placement = "b",
...
We next examined children's accuracy (i.e., their rates of putting the correct item in the bucket for two-familiar trials, in which there was a correct response). Children sometimes put two items in the bucket (`r round(tot, digits = 0)`% of trials). If they put the correct item in the bucket first followed by the incorrect object, we counted their response as correct. If they put the incorrect object in first, we counted their response as incorrect. Children's accuracy is displayed in Table 1. Overall children generally chose the correct item, indicating that they understood the task and were motivated to answer correctly.
